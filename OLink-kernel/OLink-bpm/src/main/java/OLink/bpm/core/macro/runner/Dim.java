...
public class Dim {
	 ...
	 */
	Object eventThreadMonitor = new Object();
	 ...
	 */
	private final Hashtable<String, SourceInfo> urlToSourceInfo = new Hashtable<String, SourceInfo>();

	/**
	 * Table mapping function names to information about the function.
	 */
	private final Hashtable<String, FunctionSource> functionNames = new Hashtable<String, FunctionSource>();

	/**
	 * Table mapping functions to information about the function.
	 */
	private final Hashtable<DebuggableScript, FunctionSource> functionToSource = new Hashtable<DebuggableScript, FunctionSource>();
	...
	public String[] functionNames() {
		String[] a;
		synchronized (urlToSourceInfo) {
			Enumeration<String> e = functionNames.keys();
	...
	public void clearAllBreakpoints() {
		Enumeration<SourceInfo> e = urlToSourceInfo.elements();
		while (e.hasMoreElements()) {
	...
	public void setReturnValue(int returnValue) {
		synchronized (monitor) {
			...
			// /Jarod append at here.
			// interruptedContextData = new ContextData();
	 ...
	 */
	public Script compileScript(String url, String text) {
		DimIProxy action = new DimIProxy(this, IPROXY_COMPILE_SCRIPT);
		action.url = url;
		action.text = text;
		action.withContext();
		return action.scriptResult;
	}
	...
	private void interrupted(Context cx, final StackFrame frame,
		...
		if (recursiveEventThreadCall) {
			...
			// too deep recursion of dispatchNextGuiEvent causes GUI
			// lockout.
			// Note: it can make GUI unresponsive if long-running script
			// will be called on GUI thread while processing another
			// interrupt
			/*
			if (false) {
				// Run event dispatch until gui sets a flag to exit the
				// initial
				// call to interrupted.
		...
		try {
			do {
					...
					synchronized (monitor) {
						...
						try {
							for (;;) {
								...
								if (this.returnValue != -1) { // 当其他线程改变ReturnValue时，中断循环
				...
				// Jarod 获取返回信息
				// callback.enterInterrupt(frame, threadTitle, alertMessage);
				// callback.returnDebugInfo(frame.getUrl(), frame.sourceInfo()
				// .source(), frame.getLineNumber());

			} while (false);
		} finally {
			synchronized (eventThreadMonitor) { // removed by Jarod
	...
	private static class DimIProxy implements ContextAction,
		...
		/**
		 * The object to convert, get a property from or enum1erate.
		 */
		...
		public Object run(Context cx) {
			...
			case IPROXY_COMPILE_SCRIPT:
				scriptResult = cx.compileString(text, url, 1, null);
				break;
	...
	public static class ContextData {
		...
		/**
		 * @hibernate.property column="stopAtFrameDepth"
		 */
		public int getStopAtFrameDepth() {
			return stopAtFrameDepth;
		}
	}
	...
	public static class StackFrame implements DebugFrame {
		...
		public void onLineChange(Context cx, int lineno) {
			...
			String source = this.sourceInfo().source;

			dim.callback.returnDebugInfo(this.getUrl(), source, lineno);
			// this.contextData.
			char[] dst = new char[source.length()];
			source.getChars(0, source.length(), dst, 0);
			if (lineno >= this.sourceInfo().breakableLines.length - 1) {
				dim.callback.setFinnished(true);
			} else {
				dim.callback.setFinnished(false);
			}
	...
	/**
	 * @hibernate.property column="returnValue"
	 */
