...
public class CommonUtil {
	...
	public static String getCurrentDateTime() {
		String format = "yyyy年MM月dd日 E HH点mm分ss秒";
		...
		//str = (2000 + (c.get(Calendar.YEAR) - 1900) % 100) + "年 ";
		str = str + currentDate.getMonth() + "月";
		//str = str + c.get(Calendar.MONTH) + "月";
		str = str + currentDate.getDate() + "日 ";
		//str = str + c.get(Calendar.DAY_OF_MONTH) + "日 ";
	...
	static public String getTimeStr(java.sql.Time time) {
		...
		return new SimpleDateFormat("HH:mm:00").format(time);
	...
	static public java.sql.Date strToDate(String str) {
		...
		if (!StringUtil.isBlank(str)) {
			...
			while (!str.equals("") && !str.startsWith("-")
					&& !str.startsWith(":") && !str.startsWith(" ")) {
				second.append(str.charAt(0));
				str = str.trim().substring(1);
			}
			int iyear = Integer.valueOf(year.toString()).intValue() - 1900;
			int imonth = Integer.valueOf(month.toString()).intValue() - 1;
			int idate = Integer.valueOf(date.toString()).intValue();
			int ihour = Integer.valueOf(hour.toString()).intValue();
			int iminute = Integer.valueOf(minute.toString()).intValue();
			int isecond = Integer.valueOf(second.toString()).intValue();
			java.util.Date uDate = new java.util.Date(iyear, imonth, idate,
	...
	static public java.sql.Time strToTime(String str) {
		...
		if(index < 2){
			try {
				return new java.sql.Time(new SimpleDateFormat("HH:mm:ss").parse(str).getTime());
			} catch (ParseException e) {
				e.printStackTrace();
			}
			return new java.sql.Time(0);
		} else {
			ParsePosition pp = new ParsePosition(index - 2);
			return new java.sql.Time(new SimpleDateFormat("HH:mm:ss").parse(str, pp).getTime());
		}
		/*
		String dtstr = str;
		dtstr = dtstr.replace('/', '-');
	...
	public static void main(String[] args) throws ParseException {
		...
		log.info(CommonUtil.compareTime(t1, t2));
		*/
		System.out.println(CommonUtil.getCurrDateStr());
		//System.out.println(new SimpleDateFormat("2011-02-02").parse("2011-02-02"));
