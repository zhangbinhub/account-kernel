...
public class MSSqlWFDashBoardDAO extends AbstractWFDashBoardDAO implements WFDashBoardDAO {

	public MSSqlWFDashBoardDAO(Connection conn) throws Exception {
		super(conn);
	...
	public Collection<DashBoardVO> getSumTimeData(String application, String domainid,
		...
		String sql = "SELECT B.STATELABEL NAME, ROUND(AVG(CAST(DateDiff (\"Minute\",ACTIONTIME,PROCESSTIME) AS FLOAT)/60) ,2) VALUE FROM "
	...
	public DataPackage<DashBoardVO> getSumRole(String application, String domainid,
		...
		sql = "SELECT NAME,VALUE FROM (SELECT NAME,VALUE FROM (" + sql + ") A ORDER BY NAME) B  " + startLine
				+ " ," + endLine;
