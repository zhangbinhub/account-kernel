...
public class AbortNode extends Node {

	/**
	 * 
	 */
	private static final long serialVersionUID = -4905029219915905945L;

	public AbortNode(FlowDiagram owner) {
		super(owner);
	...
	public void paintMobile(OGraphics g) {
		...
		_img = _owner.getImageResource("cancel_m.gif");
	...
	public void paint(OGraphics g) {
		if (_img == null) {
			_img = _owner.getImageResource("cancel.gif");
		}
		...
		if (this.name != null) {
			...
			g.setColor(Color.black);
			g.drawString(name, tx + 13 + name.length(), ty - 10);
		}
