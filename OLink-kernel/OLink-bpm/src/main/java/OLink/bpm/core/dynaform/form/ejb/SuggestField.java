 ...
 * 
 */
public class SuggestField extends FormField implements ValueStoreField {
	...
	public String toHtmlTxt(Document doc, IRunner runner, WebUser webUser) throws Exception {
		...
		if (displayType == PermissionType.HIDDEN) {
			...
			html.append(toAttr(doc, runner, displayType));
		}
	...
	private String toAttr(Document doc, IRunner runner, int displayType) throws Exception {
		StringBuffer html = new StringBuffer();
		Options options = getOptions(runner, doc);

		String fieldId = getFieldId(doc).replaceAll("-", "_");
		if (!TEXT_TYPE_HIDDEN.equalsIgnoreCase(getTextType())) {
			html.append("<INPUT id='show_" + getFieldId(doc) + "' ");
			html.append(" name='show_" + getName() + "'");
			html.append(" autocomplete='off'");
			html.append(" " + toOtherpropsHtml());
			if (isRefreshOnChanged()) {
				html.append(" callback='dy_refresh(this.input.id);'");
			}
			html.append(" class='input-cmd'");

			if (displayType == PermissionType.READONLY || TEXT_TYPE_READONLY.equalsIgnoreCase(getTextType())) {
				html.append(" readonly");
			}
			html.append(" />");
		}

		html.append("<INPUT type='hidden' id='" + getFieldId(doc) + "' name='" + getName() + "' resetable=true />");
		Object value = null;
		if (doc != null) {
		...
		html.append("var suggest_" + fieldId + " = null;");
		html.append("var func_" + fieldId + " = function(){suggest_" + fieldId + " = new Suggest.Local('show_"
				+ getFieldId(doc) + "', '" + getFieldId(doc) + "', " + json + ", {dispAllKey: true, defValue:'"
				+ value + "', prefix: true, highlight: true});");
		html.append("document.getElementById('show_" + getFieldId(doc) + "').ondblclick=function(){suggest_" + fieldId
				+ ".search();};};");
		html.append("Suggest.init(func_" + fieldId + ")");
		html.append("</SCRIPT>");
		}
		return html.toString();
	}
